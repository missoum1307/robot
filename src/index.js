...
const changeDirection = (direction, directionFromCommand) => {
    ...
    const newDirectionRight = allDirections[newDirectionIndex > 3 ? 0 : newDirectionIndex];
    const newDirectionLeft = allDirections[newDirectionIndex < 0 ? 3 : newDirectionIndex];
...
const renderingRobot = () => {
    ...
    const element = (index) => {
        ...
        let column = rowTarget.children[movesTrackForRending[index][0] + 1];
        return column;
    };
...
const movingForward = () => {
    ...
    oneCommand.forEach((command) => {
        ...
        if (command == "F") {
            ...
            const newXCoordinate = MoveX_Axis > roomSize || MoveX_Axis < 0 ? latestMove[0] : MoveX_Axis;
            const newYCoordinate = MoveY_Axis > roomSize || MoveY_Axis < 0 ? latestMove[1] : MoveY_Axis;
            latestMove = [newXCoordinate, newYCoordinate];
            movesTrack.push([newXCoordinate, newYCoordinate]);
...
window.addEventListener("DOMContentLoaded", () => {
    const h3 = document.getElementsByTagName("h3");
    h3[0].innerText = `End position coordinates:${JSON.stringify(currentCoordinates)}`;
